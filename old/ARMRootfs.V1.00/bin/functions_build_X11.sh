...
##############################
# 编译 libXdmcp-1.1.1
X11_LIBXDMCP=libXdmcp-1.1.1
compile_x11_libxdmcp()
{
	
	if [ ! -e $CACHEDIR/$X11_LIBXDMCP.tar.gz ]; then
		rm -rf $TEMPDIR/$X11_LIBXDMCP
		
		dispenv
		
		prepare $X11_LIBXDMCP
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static --disable-doc"
		exec_cmd "./autogen.sh $PARAM"
		exec_cmd "make install DESTDIR=$CACHEDIR/x11_libxdmcp"
		
		exec_cmd "cd $CACHEDIR/x11_libxdmcp"
		exec_cmd "tar czf $CACHEDIR/$X11_LIBXDMCP.tar.gz ."
		exec_cmd "rm -rf $CACHEDIR/x11_libxdmcp $TEMPDIR/$X11_LIBXDMCP"
	fi;
	
	PRE_REMOVE_LIST="/usr/lib/*.la /usr/lib/*.a"
	REMOVE_LIST="/usr/lib/pkgconfig"
	DEPLOY_DIST="/usr/lib"
	deploy $X11_LIBXDMCP x11_libxdmcp
}
build_x11_libxdmcp()
{	
	build_x11_util_macros
	
	run_task "构建$X11_LIBXDMCP" "compile_x11_libxdmcp"
}
##############################
...
compile_x11_xtrans()
{
	if [ ! -e $CACHEDIR/$X11_XTRANS.tar.gz ]; then
		...
		prepare $X11_XTRANS
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-doc "
		exec_cmd "./autogen.sh $PARAM"
...
compile_x11_libx11()
{
	if [ ! -e $CACHEDIR/$X11_LIBX11.tar.gz ]; then
		...
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static --enable-composecache --enable-local-transport --enable-xf86bigfont --disable-tcp-transport --enable-unix-transport --disable-ipv6 --enable-xlocale --enable-xthreads --enable-malloc0returnsnull --without-perl --enable-loadable-i18n ac_cv_func_mmap_fixed_mapped=yes"
		exec_cmd "./autogen.sh $PARAM"
		restore_native0
		exec_cmd "make -j 10"
...
compile_x11_libxt()
{
	if [ ! -e $CACHEDIR/$X11_LIBXT.tar.gz ]; then
		...
		restore_native0
		exec_cmd "make install DESTDIR=$CACHEDIR/x11_libxt -j 10"
		hide_native0
...
compile_x11_libxdamage()
{
	if [ ! -e $CACHEDIR/$X11_LIBXDAMAGE.tar.gz ]; then
		...
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static"
		exec_cmd "./autogen.sh $PARAM"
		exec_cmd "make install DESTDIR=$CACHEDIR/x11_libxdamage -j 10"
...
compile_x11_libxrender()
{
	if [ ! -e $CACHEDIR/$X11_LIBXRENDER.tar.gz ]; then
		...
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static"
		exec_cmd "./autogen.sh $PARAM"
		exec_cmd "make install DESTDIR=$CACHEDIR/x11_libxrender -j 10"
...
##############################
# 编译 /home/baiyun/Packages/libXv-1.0.7.tar.gz
X11_LIBXV=libXv-1.0.7
compile_x11_libxv()
{
	if [ ! -e $CACHEDIR/$X11_LIBXV.tar.gz ]; then
		rm -rf $TEMPDIR/$X11_LIBXV
		...
		exec_cmd "make install DESTDIR=$CACHEDIR/x11_libxv -j 10"
				
		exec_cmd "cd $CACHEDIR/x11_libxv"
		exec_cmd "tar czf $CACHEDIR/$X11_LIBXV.tar.gz ."
		exec_cmd "rm -rf $CACHEDIR/x11_libxv $TEMPDIR/$X11_LIBXV"
	fi;
	
 	PRE_REMOVE_LIST="/usr/lib/*.la /usr/lib/*.a"
	REMOVE_LIST="/usr/lib/pkgconfig"
	DEPLOY_DIST="/usr/lib"
	deploy $X11_LIBXV x11_libxv
}
build_x11_libxv()
{	
	build_x11_util_macros
	...
	run_task "构建$X11_LIBXV" "compile_x11_libxv"
}
##############################
# 编译 x11_libxext
...
compile_x11_libxext()
{
	if [ ! -e $CACHEDIR/$X11_LIBXEXT.tar.gz ]; then
		...
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static"
		exec_cmd "./autogen.sh $PARAM"
		exec_cmd "make install DESTDIR=$CACHEDIR/x11_libxext -j 10"
...
compile_x11_fontutil()
{
	if [ ! -e $CACHEDIR/$X11_FONTUTIL.tar.gz ]; then
		...
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static"
		exec_cmd "./autogen.sh $PARAM"
		exec_cmd "make install DESTDIR=$CACHEDIR/x11_fontutil -j 10"
...
##############################
# 编译 mkfontscale-1.1.0
X11_MKFONTSCALE=mkfontscale-1.1.0
compile_x11_mkfontscale()
{
	if [ ! -e $CACHEDIR/$X11_MKFONTSCALE.tar.gz ]; then
		rm -rf $TEMPDIR/$X11_MKFONTSCALE
		
		dispenv
		
		prepare $X11_MKFONTSCALE
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static --with-bzip2"
		exec_cmd "./configure $PARAM"
		exec_cmd "make install DESTDIR=$CACHEDIR/x11_mkfontscale -j 10"
				
		exec_cmd "cd $CACHEDIR/x11_mkfontscale"
		exec_cmd "tar czf $CACHEDIR/$X11_MKFONTSCALE.tar.gz ."
		exec_cmd "rm -rf $CACHEDIR/x11_mkfontscale $TEMPDIR/$X11_MKFONTSCALE"
	fi;
	
 	PRE_REMOVE_LIST=""
	REMOVE_LIST=""
	DEPLOY_DIST="/usr/bin"
	deploy $X11_MKFONTSCALE x11_mkfontscale
}
build_x11_mkfontscale()
{	
	build_x11_util_macros
	build_libz
	build_libbz2
	...
	run_task "构建$X11_MKFONTSCALE" "compile_x11_mkfontscale"
}

##############################
# 编译 mkfontdir-1.0.7
X11_MKFONTDIR=mkfontdir-1.0.7
compile_x11_mkfontdir()
{
	if [ ! -e $CACHEDIR/$X11_MKFONTDIR.tar.gz ]; then
		rm -rf $TEMPDIR/$X11_MKFONTDIR
		
		dispenv
		
		prepare $X11_MKFONTDIR
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static"
		exec_cmd "./configure $PARAM"
		exec_cmd "make install DESTDIR=$CACHEDIR/x11_mkfontdir -j 10"
				
		exec_cmd "cd $CACHEDIR/x11_mkfontdir"
		exec_cmd "tar czf $CACHEDIR/$X11_MKFONTDIR.tar.gz ."
		exec_cmd "rm -rf $CACHEDIR/x11_mkfontdir $TEMPDIR/$X11_MKFONTDIR"
	fi;
	
 	PRE_REMOVE_LIST=""
	REMOVE_LIST=""
	DEPLOY_DIST="/usr/bin"
	deploy $X11_MKFONTDIR x11_mkfontdir
}
build_x11_mkfontdir()
{	
	build_x11_util_macros
	
	run_task "构建$X11_MKFONTDIR" "compile_x11_mkfontdir"
}
...
compile_x11_libXfont()
{
	if [ ! -e $CACHEDIR/$X11_LIBXFONT.tar.gz ]; then
		...
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static --with-bzip2 --enable-local-transport --enable-freetype"
		exec_cmd "./autogen.sh $PARAM"
...
compile_x11_libxft()
{
	if [ ! -e $CACHEDIR/$X11_LIBXFT.tar.gz ]; then
		...
		prepare $X11_LIBXFT
		PARAM="--prefix=/usr --host=$MY_TARGET --sysconfdir=/etc --disable-static"
		exec_cmd "./autogen.sh $PARAM"
		exec_cmd "make V=1 -j 10"
...
#  --disable-mitshm        Build SHM extension (default: enabled)
...
#                          disabled)
		prepare $XORG_SERVER
		PARAM="--prefix=/usr --host=$MY_TARGET --disable-static --enable-option-checking"			# 基本设置
		PARAM+=" --disable-dependency-tracking"														# speeds up one-time build
		PARAM+=" --disable-docs --disable-devel-docs --disable-unit-tests --disable-debug"			# 不编译文档/单元测试代码/调试代码
		PARAM+=" --disable-sparkle"																	# Sparkle: a free software update framework for the Mac
		PARAM+=" --enable-use-sigio-by-default"
		PARAM+=" --enable-xorg --enable-composite  --enable-kdrive --enable-xfbdev --enable-registry --enable-mitshm --enable-vgahw"
		PARAM+=" --enable-dga --enable-xaa"															# DGA: Direct Graphics Access, XAA: EXA的前称
		PARAM+=" --disable-ipv6 --disable-tcp-transport --enable-local-transport"					# 传输协议
		PARAM+=" --enable-xf86bigfont"																# 大字体，需要libX11同时支持
		PARAM+=" --enable-config-udev --disable-config-hal --disable-config-dbus"					# 即插即用
		PARAM+=" --enable-xdmcp"																	# XDMCP（X Display Manager Control Protocol） X显示监控协议
		
		### 不使用的功能
		PARAM+=" --disable-xinerama"																# XINERAMA : 多屏显示( Xinerama is an extension to the X Window System that enables X applications and window managers to use two or more physical displays as one large virtual)
		...
		PARAM+=" --disable-dpms"			######################################################### DPMS (Display Power Management Signaling)，这个模块的freescale驱动有问题
