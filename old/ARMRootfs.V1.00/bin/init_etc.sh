...
busybox_init_network()
{
	...
	sudo tar axf $BUILDINDIR/netbase-from-ubuntu.tar* -C $INSTDIR	|| exit 1;	# /etc/protocols和services两个文件
...
_MY_EOF_
	...
	tar jxf $BUILDINDIR/etc-prg.tar.bz2 50default || exit 2;
	
	echo "......3. "/etc/init.d/network
	mkdir -p $INSTDIR/etc/init.d || exit 3;
	cd $INSTDIR/etc/init.d
	tar jxf $BUILDINDIR/etc-prg.tar.bz2 networking || exit 3;
...
#	sed -i 's/\#\!\/bin\/sh/\#\!\/bin\/sh\necho \"-------------Starting Network--------------\"\n\/etc\/init.d\/networking restart\n/g' $INSTDIR/etc/init.d/rcS || exit 4;
	...
	tar jxf $BUILDINDIR/etc-prg.tar.bz2 interfaces || exit 4; 
}
